define(
    "./workbox-b9733a6e.js", [ "exports" ], (function(t) {
      "use strict";
      try {
      ...
      class s extends Error {
        constructor(t, s) { super(e(t, s)), this.name = t, this.details = s }
      }
      try {
      ...
      class a {
        ...
        addFetchListener() {
          self.addEventListener(
              "fetch", (t => {
                const {request : e} = t, s = this.handleRequest(
                                             {request : e, event : t});
                s && t.respondWith(s)
              }))
        }
          ...
          self.addEventListener(
              "message", (t => {
                if (t.data && "CACHE_URLS" === t.data.type) {
                  ...
                  t.waitUntil(s), t.ports && t.ports[0] &&
        ...
        handleRequest({request : t, event: e}) {
          ...
          let a = r && r.handler;
          const c = t.method;
          if (!a && this.i.has(c) && (a = this.i.get(c)), !a)
            return;
          let o;
          try {
          ...
          return o instanceof Promise && (this.o || h) &&
                     (o = o.catch((async n => {
                       if (h)
                         try {
        ...
        findMatchingRoute({url : t, sameOrigin: e, request: s, event: n}) {
          ...
          for (const r of i) {
            let i;
            const a =
                r.match({url : t, sameOrigin : e, request : s, event : n});
            if (a)
              return i = a,
      ...
      const u = {
        ...
        suffix : "undefined" != typeof registration ? registration.scope : ""
            ...
            l = t => [u.prefix, t, u.suffix]
                         .filter((t => t && t.length > 0))
                         .join("-"),
            f = t => t || l(u.precache), w = t => t || l(u.runtime);
      function d(t) { t.then((() => {})) }
      const p = new Set;
      class y {
        constructor(t, e, {onupgradeneeded : s, onversionchange: n} = {}) {
          this.h = null, this.u = t, this.l = e, this.p = s,
          this.m = n || (() => this.close())
        }
        get db() { return this.h }
        async open() {
          if (!this.h)
            return this.h = await new Promise(((t, e) => {
                     let s = !1;
                     setTimeout(
                         (() => {
                           s = !0,
                     ...
                     n.onerror = () => e(n.error), n.onupgradeneeded = t => {
                       s ? (n.transaction.abort(), n.result.close())
                         : "function" == typeof this.p && this.p(t)
                     }, n.onsuccess = () => {
                       const e = n.result;
                       s ? e.close()
                         : (e.onversionchange = this.m.bind(this), t(e))
                     }
                   })),
                   this
        }
        ...
        async getAllKeys(t, e, s) {
          return (await this.getAllMatching(
                      t, {query : e, count : s, includeKeys : !0}))
              .map((t => t.key))
        }
        async getAllMatching(t, {
          ...
          return await this.transaction(
              [ t ], "readonly", ((a, c) => {
                const o = a.objectStore(t), h = e ? o.index(e) : o, u = [],
                      l = h.openCursor(s, n);
                l.onsuccess = () => {
                  const t = l.result;
                  t ? (u.push(r ? t : t.value),
        ...
        async transaction(t, e, s) {
          return await this.open(), await new Promise(((n, i) => {
                   ...
                   r.onabort = () => i(r.error), r.oncomplete = () => n(),
                   s(r, (t => n(t)))
                 }))
        }
        async g(t, e, s, ...n) {
        ...
        close() { this.h && (this.h.close(), this.h = null) }
      }
      ...
      for (const [t, e] of Object.entries(m))
        for (const s of e)
          s in IDBObjectStore.prototype &&
              (y.prototype[s] = async function(
                   e, ...n) { return await this.g(s, e, t, ...n) });
      try {
        self["workbox:expiration:6.1.5"] && _()
      } catch (t) {
      }
      const g = "cache-entries", R = t => {
      ...
      class v {
        ...
        v(t) {
          ...
          e.createIndex("cacheName", "cacheName", {unique : !1}),
              ...
              (async t => {await new Promise(((e, s) => {
                 const n = indexedDB.deleteDatabase(t);
                 n.onerror = () => { s(n.error) },
                 n.onblocked = () => { s(new Error("Delete blocked")) },
                 n.onsuccess = () => { e() }
               }))})(this.R)
        }
        ...
        async expireEntries(t, e) {
          const s = await this.h.transaction(
              g, "readwrite", ((s, n) => {
                const i = s.objectStore(g)
                              .index("timestamp")
                ...
                i.onsuccess = () => {
                  ...
                  if (s) {
                    const n = s.value;
                    n.cacheName === this.R &&
                        (t && n.timestamp < t || e && a >= e ? r.push(s.value)
                                                             : a++),
                        s.continue()
                  } else
                    n(r)
                ...
                n = [];
          for (const t of s)
            await this.h.delete(g, t.id), n.push(t.url);
          return n
        }
        q(t) { return this.R + "|" + R(t) }
      }
      class q {
        constructor(t, e = {}) {
          this.U = !1, this.L = !1, this._ = e.maxEntries,
          this.N = e.maxAgeSeconds, this.C = e.matchOptions, this.R = t,
          this.D = new v(t)
        }
        async expireEntries() {
          if (this.U)
          ...
          const t = this.N ? Date.now() - 1e3 * this.N : 0,
                e = await this.D.expireEntries(t, this._),
                s = await self.caches.open(this.R);
          for (const t of e)
            await s.delete(t, this.C);
          this.U = !1, this.L && (this.L = !1, d(this.expireEntries()))
        }
        async updateTimestamp(t) { await this.D.setTimestamp(t, Date.now()) }
        ...
        async delete() { this.L = !1, await this.D.expireEntries(1 / 0) }
      }
      function U() {
      ...
      function x(t) { return "string" == typeof t ? new Request(t) : t }
      class N {
        constructor(t, e) {
          this.T = {}, Object.assign(this, e), this.event = e.event, this.k = t,
          this.K = new L, this.O = [], this.P = [...t.plugins ],
          this.M = new Map;
        ...
        async fetch(t) {
          const {event : e} = this;
          let n = x(t);
          if ("navigate" === n.mode && e instanceof FetchEvent &&
              e.preloadResponse) {
          ...
          try {
            let t;
            t = await fetch(n, "navigate" === n.mode ? void 0
                                                     : this.k.fetchOptions);
        ...
        async cachePut(t, e) {
          ...
          if (!e)
            throw new s("cache-put-with-no-response", {
              url : (a = r.url,
                     new URL(String(a), location.href)
                         .href.replace(new RegExp(`^${location.origin}`), ""))
            });
          var a;
          const c = await this.A(e);
          if (!c)
            return !1;
          const {
            cacheName : o,
         ...
         u = await self.caches.open(o), l = this.hasCallback("cacheDidUpdate"),
         f = l ? await async function(t, e, s, n) {
           const i = b(e.url, s);
           if (e.url === i)
             return t.match(e, n);
           const r = U({}, n, {ignoreSearch : !0}), a = await t.keys(e, r);
           for (const e of a)
             if (i === b(e.url, s))
               return t.match(e, n)
        ...
        * iterateCallbacks(t) {
        ...
        waitUntil(t) { return this.O.push(t), t }
        async doneWaiting() {
          let t;
          for (; t = this.O.shift();)
            await t
        }
        destroy() { this.K.resolve() }
        async A(t) {
          let e = t, s = !1;
      ...
      class C {
        constructor(t = {}) {
          this.cacheName = w(t.cacheName), this.plugins = t.plugins || [],
          this.fetchOptions = t.fetchOptions, this.matchOptions = t.matchOptions
        }
        handle(t) {
        ...
        handleAll(t) {
          ...
          const e = t.event,
                s = "string" == typeof t.request ? new Request(t.request)
                                                 : t.request,
      ...
      class T {
        constructor() {
          ...
          this.handlerWillStart = async (
              {request : t, state : e}) => { e && (e.originalRequest = t) },
          this.cachedResponseWillBeUsed =
              async ({event : t, state : e, cachedResponse : s}) => {
            if ("install" === t.type) {
      ...
      class k {
        constructor({precacheController : t}) {
          this.cacheKeyWillBeUsed = async ({request : t, params : e}) => {
            const s = e && e.cacheKey || this.F.getCacheKeyForURL(t.url);
      ...
      class M extends C {
        constructor(t = {}) {
          t.cacheName = f(t.cacheName), super(t),
          this.I = !1 !== t.fallbackToNetwork,
          this.plugins.push(M.copyRedirectedCacheableResponsesPlugin)
        }
        async j(t, e) {
          const s = await e.cacheMatch(t);
          return s ||
                 (e.event && "install" === e.event.type ? await this.B(t, e)
                                                        : await this.H(t, e))
        }
        async H(t, e) {
          let n;
          ...
          for (const [s, n] of this.plugins.entries())
            n !== M.copyRedirectedCacheableResponsesPlugin &&
                (n === M.defaultPrecacheCacheabilityPlugin && (t = s),
                 n.cacheWillUpdate && e++);
          0 === e ? this.plugins.push(M.defaultPrecacheCacheabilityPlugin)
      ...
      M.copyRedirectedCacheableResponsesPlugin = {
        cacheWillUpdate : async ({response : t}) =>
            t.redirected ? await P(t) : t
      };
      class A {
        constructor(
            {cacheName : t, plugins: e = [], fallbackToNetwork: s = !0} = {}) {
          this.G = new Map, this.V = new Map, this.J = new Map,
          this.k = new M({
            cacheName : f(t),
        ...
        get strategy() { return this.k }
        precache(t) {
          this.addToCacheList(t),
              this.X || (self.addEventListener("install", this.install),
          ...
          for (const n of t) {
            ...
            if (this.G.has(i) && this.G.get(i) !== t)
              throw new s("add-to-cache-list-conflicting-entries",
                          {firstEntry : this.G.get(i), secondEntry : t});
        ...
        install(t) {
          return E(t, (async () => {
                     ...
                     for (const [e, s] of this.G) {
                       const n = this.J.get(s), i = this.V.get(e),
                             r = new Request(e, {
                               integrity : n,
                               cache : i,
        ...
        activate(t) {
          return E(t, (async () => {
                     const t = await self.caches.open(this.strategy.cacheName),
                           e = await t.keys(), s = new Set(this.G.values()),
                           n = [];
                     for (const i of e)
        ...
        getURLsToCacheKeys() { return this.G }
        getCachedURLs() { return [...this.G.keys() ] }
        getCacheKeyForURL(t) {
          const e = new URL(t, location.href);
      ...
      , t.ExpirationPlugin = class {
        constructor(t = {}) {
          ...
          this.cacheDidUpdate =
              async ({cacheName : t, request : e}) => {
            const s = this.Z(t);
            await s.updateTimestamp(e.url), await s.expireEntries()
          },
          this.tt = t, this.N = t.maxAgeSeconds, this.et = new Map,
          t.purgeOnQuotaError &&
      ...
      , t.precacheAndRoute = function(t, e) {
        !function(t) { S().precache(t) }(t), function(t) {
          const e = S();
          h(new W(e, t))
        }(e)
      }, t.registerRoute = h
    }));
//# sourceMappingURL=workbox-b9733a6e.js.map
